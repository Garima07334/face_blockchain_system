...
class VerificationPipeline:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        max_candidates:
            None = default to first 3 candidates.
            Positive integer = evaluate only that many candidates.
        """

        # --------------------------------------------------------------
        # SHARED FACE COMPONENTS
        # --------------------------------------------------------------
        ...
        # IMPORTANT:
        # Pass the SAME encoder/detector into FaceMatcher.
        #
        # This prevents another SFace model from being loaded.
        self.face_matcher = (
            face_matcher
            or FaceMatcher(
                detector=self.detector,
    ...
    def run_pipeline(
        ...
        print(
            "FACE VERIFICATION PIPELINE STARTED"
        )
        ...
        print(
            "\n[PHASE 2] SFace query encoding"
        )

        encoding = (
            self.encoder.encode_face(
                image_path,
                face_index=face_index,
            )
        )
        ...
        print(
            "\n[PHASE 3] Google Lens reverse image search"
        )
        ...
        search_response = (
            self.web_search.search_by_image(
                image_path
            )
        )
        ...
        # ============================================================== 
        # PHASE 4 — NORMALIZE SEARCH RESULTS
        # ==============================================================

        phase_started = time.perf_counter()

        print(
            "\n[PHASE 4] Normalizing search results"
        )

        selection = (
            self.result_selector.select_results(
                search_response,
                max_results=None,
            )
        )
        ...
        print(
            f"[PHASE 4] Normalized candidates: "
            f"{len(all_candidates)}"
        )

        print(
            f"[PHASE 4] Candidates to evaluate: "
            f"{len(candidates_to_evaluate)}"
        )
        ...
        for evaluation_index, candidate in enumerate(
            ...
            if image_url:
                ...
                print(
                    "[CANDIDATE] Verifying original image..."
                )
            ...
            # IMPORTANT:
            #
            # Thumbnail is ONLY tried when the original
            # produced a technical verification error.
            #
            # NO_MATCH is a valid biometric result.
            # Therefore we DO NOT run another SFace
            # verification for NO_MATCH.
            ...
            if should_try_thumbnail:
                ...
                print(
                    "[CANDIDATE] Original failed technically."
                )

                print(
                    "[CANDIDATE] Trying thumbnail fallback..."
                )
            ...
            if (
                ...
                print(
                    "[CANDIDATE] Technical verification error."
                )

                print(
                    f"[CANDIDATE] Time: "
                    f"{time.perf_counter() - candidate_started:.2f}s"
                )
        ...
        print(
            "\n[PHASE 5] Selecting best biometric candidate..."
        )
        ...
        if not face_match_confirmed:
            ...
            print(
                "\n[SECURITY] NO SAME-FACE MATCH."
            )

            print(
                "[SECURITY] Blockchain registration BLOCKED."
            )
        ...
        print(
            "\n[SECURITY] SAME FACE MATCH CONFIRMED."
        )

        print(
            "[SECURITY] Blockchain registration allowed."
        )
        ...
        if not self.blockchain.is_connected():
            ...
            print(
                "[PHASE 7] Blockchain unavailable."
            )
        ...
        print(
            f"[PHASE 8] Registration complete "
            f"({time.perf_counter() - phase_started:.2f}s)"
        )
        ...
        print(
            f"[PHASE 9] Verification complete "
            f"({time.perf_counter() - phase_started:.2f}s)"
        )
        ...
        print(
            f"PIPELINE FINISHED IN {total_time:.2f} SECONDS"
        )
